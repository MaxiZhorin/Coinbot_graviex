...
class Auth:
    ...
    def sign(self, verb, path, param=None):
...
class Client:
    ...
    def get(self, path, param=None):
        verb = "GET"
        signature, query = self.auth.sign_params(verb, path, param)
...
# апи токен и секрет токен
client = Client(access_key='z81oksjCIVYdZqOnS79c4qnCbY7T9oYc0x4qNF3s',
                secret_key='mft9K1Ks02oMabnzIaanYRIuZq0HVkLIe0IYMWYs')


def order_price_act():
    orders = client.get(get_api_path('order_book'), param={'market': order_market})
                ...
                if order_val == order_val2:
                    ...
                    if order_price < buy_value:
                        if origin_value >= float(buy_value):
                            ...
                            params = {'market': order_market, 'side': 'sell', 'volume': buy_value_new,
                                      'price': price_for_orders_new}
                            res = client.post(get_api_path('orders'), params)
                            order_id = int(res['id'])
                            order_clear.append(order_id)
                            print res
                            origin_value = 0
                        print 'ok3'
                        params = {'market': order_market, 'side': 'buy', 'volume': order_price,
                                  'price': price_for_orders}
                        res = client.post(get_api_path('orders'), params)
